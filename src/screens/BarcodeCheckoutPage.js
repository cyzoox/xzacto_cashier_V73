...
const BarcodeCheckoutScreen = ({ navigation, route }) => {
  const store_info = route.params.store_info;
  const products_list = route.params.store_info;
  ...
  return (
    <View style={{ flex: 1 }}>
      <Alert
        visible={nocustomer}
        onCancel={onCancelNoCus}
        onProceed={onProceedNoCus}
        title="No selected customer"
        content="For Credit transactions you need to select customer."
        confirmTitle="OK"
      />
      <Alert
        visible={creditVisible}
        onCancel={onCancelCredit}
        onProceed={onProceedCredit}
        title="Proceed credit payment?"
        content="Are you sure you want to proceed with credit payment?"
      ...
      <Alert
        ...
        content="Cash receive is lesser than the total amount."
        confirmTitle="OK"
      />
      <AlertwithChild
      ...
      <AlertwithChild
        visible={autoPrint}
        onCancel={() => setAutoPrint(false)}
        onProceed={() => setAutoPrint(false)}
        title="Set Auto Printing"
        confirmTitle="S A V E"
      >
        <View
          style={{
      ...
      <AppHeader
        ...
        leftComponent={
          <TouchableOpacity
            onPress={() => {
              setDiscount(0), navigation.goBack();
            }}
          >
            <EvilIcons name={"arrow-left"} size={35} color={colors.white} />
          </TouchableOpacity>
        }
        rightComponent={<CustomerScreen selectedCustomer={customer} />}
      />
      {name ? (
        <Card
          containerStyle={{
            ...
            paddingTop: 5,
            paddingBottom: 5,
            backgroundColor: colors.statusBarCoverDark,
        ...
        >
          ...
          >
            <Text
              style={{ color: colors.accent, fontSize: 17, fontWeight: "700" }}
            >
              Customer:
            </Text>
            <Text
      ...
      <Grid>
        <Row size={1.5}>
          <List
            toggleScanner={setScanner}
            screen={route.name}
            discount_visible={setDiscountVisible}
            discount={selected}
            autoPrint_visible={setAutoPrint}
          />
        </Row>
        <Row>
          <AmountKeys
...
export default BarcodeCheckoutScreen;
